...
char Load3DS (obj_type_ptr p_object, char *p_filename)
{
	...
	unsigned short l_chunk_id, s_temp; //Chunk identifier
	unsigned int l_chunk_lenght; //Chunk lenght
 	unsigned char l_char; //Char variable
	unsigned short l_qty; //Number of elements in each chunk

	unsigned short l_face_flags; //Flag that stores some face information
    printf("reading file........ %s",p_filename);
	//getchar();
	if ((l_file=fopen (p_filename, "rb"))== NULL) return 0; //Open the file
         
	printf("reading file........ %s..........filelength %d...\n",p_filename,filelength (fileno (l_file)));	
                ...
                //printf("Number of polygons: %x\n",l_qty); 
					for (i=0; i<l_qty; i++)
					{
						//fread (&p_object->polygon[i].a, sizeof (unsigned short), 1, l_file);
               			fread (&s_temp, sizeof (unsigned short), 1, l_file);
